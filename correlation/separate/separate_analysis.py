...
excel_path = 'population'
image_path = 'population_result'
...
for filename in os.listdir(excel_path):
    ...
    slice_names = ["细粒度数量与人口密度的相关性", "细粒度面积与人口密度的相关性", "细粒度密度与人口密度的相关性", "细粒度占比与人口密度的相关性",
                   "细粒度比例与人口密度的相关性", "建筑物高度与人口密度的相关性", "建筑物面积数量与人口密度的相关性"]
    ...
    for idx, s in enumerate(slices):
        ...
        image_name = filename[:-5]
        title = f"{image_name}-{slice_names[idx]}"
        plt.title(title)
        # 保存当前柱状图
        plt.savefig(f'{image_path}/Spearman/{image_name}-{slice_names[idx]}.png')
